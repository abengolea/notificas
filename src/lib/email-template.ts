// Template base para emails de notificación
...
export function generateEmailHtml(params: {
  ...
  return emailTemplate
    .replace(/\{\{senderName\}\}/g, params.senderName)
    .replace(/\{\{recipientName\}\}/g, params.recipientName)
    .replace(/\{\{recipientEmail\}\}/g, params.recipientEmail)
    .replace(/\{\{readUrl\}\}/g, params.readUrl)
    .replace(/\{\{fallbackUrl\}\}/g, params.fallbackUrl)
    .replace(/\{\{year\}\}/g, params.year.toString());
}

// Función para generar email con tracking integrado (para usar en Firebase Functions)
export function generateEmailWithTracking(params: {
  senderName: string;
  recipientName: string;
  recipientEmail: string;
  readUrl: string;
  ...
  const { docId, trackingToken, trackingBaseUrl, ...emailParams } = params;
  
  // Generar el HTML base
  let html = generateEmailHtml(emailParams);
