...
const TraceabilityItem = ({ title, timestamp, data, statusIcon }: { title: string, timestamp?: Date, data: { label: string, value: string, icon: React.ReactNode }[], statusIcon: React.ReactNode }) => {
  ...
  const handleCopy = (value: string) => {
    ...
    toast({ title: '¡Copiado al portapapeles!', variant: 'default' });
  };
...
export default function BfaTraceability({ message }: BfaTraceabilityProps) {
  const sentData = [
    ...
    { label: 'Verificar', value: message.bfaEnviado.verificacionUrl, icon: <CheckCircle2 className="h-4 w-4" /> },
  ];
  ...
  const readData = message.bfaLeido ? [
    ...
    { label: 'Dirección IP', value: message.bfaLeido.ipLector, icon: <MousePointerClick className="h-4 w-4" /> },
    { label: 'Dispositivo', value: message.bfaLeido.dispositivoLector.substring(0,25)+'...', icon: <FileText className="h-4 w-4" /> },
    { label: 'Verificar', value: message.bfaLeido.verificacionUrl, icon: <CheckCircle2 className="h-4 w-4" /> },
  ] : [];
  
  return (
    <Card>
      <CardHeader>
        <CardTitle>Trazabilidad Certificada</CardTitle>
      </CardHeader>
      <CardContent>
