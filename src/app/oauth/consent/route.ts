...
import { listOrgsForAdminEmail, resolveAuthorizedOrg, resolveOrgForCrmProductScopes } from "@/mcp/auth/orgs";
...
import { parseCrmScopeString, crmMcpRequiresCompanyOrg, CrmInvalidScopeError } from "@/mcp/crm/scopes";
...
export async function GET(request: NextRequest) {
  ...
  const organizations = adminEmail ? await listOrgsForAdminEmail(adminEmail) : [];
  return oauthCorsResponse(
    { client_name: client.clientName, client_id: client.id, organizations },
    200,
  );
}
...
export async function POST(request: NextRequest) {
  ...
  let tokenOrgId = isCrmResource ? workspaceId : org!.id;
  let tokenOrgName = isCrmResource ? "Notificas CRM" : org!.nombre;
  let tokenSenderUid = isCrmResource ? decoded.uid : org!.adminUserId;
  let tokenSenderEmail = isCrmResource ? decoded.email || "" : org!.adminUserEmail;

  if (isCrmResource && crmMcpRequiresCompanyOrg(scopes)) {
    if (!orgId) {
      return oauthCorsResponse(
        {
          error: "invalid_request",
          error_description: "org_id is required when requesting notification or certificate scopes.",
        },
        400,
      );
    }
    const company = await resolveOrgForCrmProductScopes(decoded.email, orgId);
    if (!company) {
      return oauthCorsResponse({ error: "access_denied", error_description: "You cannot authorize this company." }, 403);
    }
    tokenOrgId = company.id;
    tokenOrgName = company.nombre;
    tokenSenderUid = company.adminUserId || decoded.uid;
    tokenSenderEmail = company.adminUserEmail;
  }
  ...
  const code = await createAuthorizationCode({
    ...
    orgId: tokenOrgId,
    orgName: tokenOrgName,
    senderUid: tokenSenderUid,
    senderEmail: tokenSenderEmail,
